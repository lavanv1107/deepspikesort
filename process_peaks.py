...
def filter_peaks(recording, peaks):
    ...
    """
    # Columns to keep
    columns_filtered = ['sample_index', 'channel_index']

    # Define a new dtype with only the desired columns
    dtype_filtered = np.dtype([(name, peaks.dtype[name]) for name in columns_filtered])

    # Create a new array with the new dtype
    peaks_filtered = np.zeros(peaks.shape, dtype=dtype_filtered)
    ...
    mask_filtered = (peaks['sample_index'] >= 31) & (peaks['sample_index'] <= recording.get_num_frames() - 33)

    peaks_filtered = peaks_filtered[mask_filtered]
